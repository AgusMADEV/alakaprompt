...
PALETTE = {
    'background': '#f8f8f0',    # Fondo principal
    'light':      '#f8f898',    # Acento claro
    'accent':     '#e0b848',    # Acento principal
    'foreground': '#303030'     # Texto oscuro
}
...
def select_project_folder():
    ...
    if folder:
        ...
        lbl_selected_folder.config(text=folder)
...
def generar_prompt_for_folder(project_folder):
    """Genera el prompt para un folder específico (usado para proyectos 'agusmadev-')."""
    prompt = "Crea / modifica un software informático en base a los parámetros que a continuación te voy a indicar:\n\n"
...
def save_prompts_for_agusmadev_projects():
    """
    Busca subcarpetas que comiencen con 'agusmadev-' en la carpeta seleccionada y guarda el prompt
    ...
    for entry in os.listdir(selected_project_folder):
        ...
        if os.path.isdir(full_path) and entry.startswith("agusmadev-"):
    ...
    if saved_count > 0:
        ...
        messagebox.showinfo("Sin proyectos", "No se encontraron carpetas que comiencen con 'agusmadev-'.")

# --- Configuración de la ventana principal ---
style = ttk.Style('flatly')
# --- Ajuste de la paleta sobre flatly ---
# Fondo / texto por defecto
style.configure('.', 
...
# Buttons (con hover claro y texto de disabled)
...
# Entradas de texto multilinea y simples
style.configure('TEntry',
    fieldbackground=PALETTE['background'], 
    foreground=PALETTE['foreground']
)

# Radio y Checkbuttons
...
# (continúa con tu código de icono y geometry)
...
def crear_campo(master, label_text, desc_text, height=3):
    ...
    # Aquí aplicamos el bg/fg directamente al tk.Text
    txt = tk.Text(frame, height=height,
                  bg=PALETTE['background'],
                  fg=PALETTE['foreground'],
                  insertbackground=PALETTE['foreground']  # cursor
    )
...
# Etiqueta para mostrar la ruta seleccionada debajo del botón, con color más claro
lbl_selected_folder = ttk.Label(
    frame_left, text="Ninguna carpeta seleccionada", foreground="grey70"
)
lbl_selected_folder.pack(pady=(0,5), fill=tk.X)
...
btn_save_prompts = ttk.Button(frame_left, text="Guardar Prompts para proyectos agusmadev-", bootstyle=SUCCESS, command=save_prompts_for_agusmadev_projects)
